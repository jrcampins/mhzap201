...
public class AsistentePaginaActualizacionFiltroClaRecVinPorClaseRecursoPar {

    private PaginaActualizacionFiltroClaRecVinPorClaseRecursoPar bean;

    public AsistentePaginaActualizacionFiltroClaRecVinPorClaseRecursoPar() {
        this.bean = null;
    }

    public AsistentePaginaActualizacionFiltroClaRecVinPorClaseRecursoPar(PaginaActualizacionFiltroClaRecVinPorClaseRecursoPar bean) {
        this.bean = bean;
    }
    ...
    public Option[] getOpcionesListaFuncionEdicion() {
        ...
        return new Option[]{ //
                ...
                //  new Option(FiltroClaRecVinCachedRowSetDataProvider2.FUNCION_MODIFICAR_FILTRO_CLA_REC_VIN, BundleWebui.getString("editar"))
                };
    }
    ...
    protected GestorBusquedaReferencia getGestorBusquedaIdClaseRecursoParColumna() {
        if (this.gestorBusquedaIdClaseRecursoParColumna == null) {
            this.gestorBusquedaIdClaseRecursoParColumna = new GestorBusquedaReferencia(
                    bean.getFiltroClaRecVinDataProvider(),
                    bean.getClaseRecursoParReferenceDataProvider(),
                    FiltroClaRecVinCachedRowSetDataProvider2.COLUMNA_ID_CLASE_RECURSO_PAR_COLUMNA,
    ...
    public String getTextoCampoIdClaseRecursoParColumna1() {
        ...
        if (bean.getGestor().isFilaNoProcesada()) {
            return STP.getString(bean.getFiltroClaRecVinDataProvider().getIdClaseRecursoParColumna(rowKey));
        }
        this.getGestorBusquedaIdClaseRecursoParColumna().setFiltroBusqueda(this.getFiltroBusquedaIdClaseRecursoParColumna());
...
//      Long id = bean.getFiltroClaRecVinDataProvider().getIdClaseRecursoParColumna(rowKey);
//      return bean.getGestor().getScriptAbrirVentanaVerDetalle(urx, id);
    ...
    protected GestorBusquedaReferencia getGestorBusquedaIdClaseRecursoParValor() {
        if (this.gestorBusquedaIdClaseRecursoParValor == null) {
            this.gestorBusquedaIdClaseRecursoParValor = new GestorBusquedaReferencia(
                    bean.getFiltroClaRecVinDataProvider(),
                    bean.getClaseRecursoParReferenceDataProvider(),
                    FiltroClaRecVinCachedRowSetDataProvider2.COLUMNA_ID_CLASE_RECURSO_PAR_VALOR,
    ...
    public String getTextoCampoIdClaseRecursoParValor1() {
        ...
        RowKey rowKey = bean.getGestor().getCurrentRowKey();
        if (bean.getGestor().isFilaNoProcesada()) {
            return STP.getString(bean.getFiltroClaRecVinDataProvider().getIdClaseRecursoParValor(rowKey));
        }
        this.getGestorBusquedaIdClaseRecursoParValor().setFiltroBusqueda(this.getFiltroBusquedaIdClaseRecursoParValor());
...
//      RowKey rowKey = bean.getGestor().getCurrentRowKey();
//      String urx = URX2.DETALLE_CLASE_RECURSO_PAR;
//      Long id = bean.getFiltroClaRecVinDataProvider().getIdClaseRecursoParValor(rowKey);
//      return bean.getGestor().getScriptAbrirVentanaVerDetalle(urx, id);
        return null;
    }
    ...
    public boolean isNullNumeroOperadorCom() {
        ...
        RowKey rowKey = bean.getGestor().getCurrentRowKey();
        Integer value = bean.getFiltroClaRecVinDataProvider().getNumeroOperadorCom(rowKey);
        return value == null;
    }
    ...
    public boolean isGridIdFiltroClaRecVinRendered() {
        return true;
    }
