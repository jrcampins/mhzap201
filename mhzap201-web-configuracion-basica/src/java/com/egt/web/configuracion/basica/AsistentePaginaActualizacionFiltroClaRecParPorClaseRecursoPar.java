...
public class AsistentePaginaActualizacionFiltroClaRecParPorClaseRecursoPar {

    private PaginaActualizacionFiltroClaRecParPorClaseRecursoPar bean;

    public AsistentePaginaActualizacionFiltroClaRecParPorClaseRecursoPar() {
        this.bean = null;
    }

    public AsistentePaginaActualizacionFiltroClaRecParPorClaseRecursoPar(PaginaActualizacionFiltroClaRecParPorClaseRecursoPar bean) {
        this.bean = bean;
    }
    ...
    public Option[] getOpcionesListaFuncionEdicion() {
        ...
        return new Option[]{ //
                ...
                //  new Option(FiltroClaRecParCachedRowSetDataProvider2.FUNCION_MODIFICAR_FILTRO_CLA_REC_PAR, BundleWebui.getString("editar"))
                };
    }
    ...
    protected GestorBusquedaReferencia getGestorBusquedaIdClaseRecursoParColumna() {
        if (this.gestorBusquedaIdClaseRecursoParColumna == null) {
            this.gestorBusquedaIdClaseRecursoParColumna = new GestorBusquedaReferencia(
                    bean.getFiltroClaRecParDataProvider(),
                    bean.getClaseRecursoParReferenceDataProvider(),
                    FiltroClaRecParCachedRowSetDataProvider2.COLUMNA_ID_CLASE_RECURSO_PAR_COLUMNA,
    ...
    public String getTextoCampoIdClaseRecursoParColumna1() {
        ...
        if (bean.getGestor().isFilaNoProcesada()) {
            return STP.getString(bean.getFiltroClaRecParDataProvider().getIdClaseRecursoParColumna(rowKey));
        }
        this.getGestorBusquedaIdClaseRecursoParColumna().setFiltroBusqueda(this.getFiltroBusquedaIdClaseRecursoParColumna());
...
//      Long id = bean.getFiltroClaRecParDataProvider().getIdClaseRecursoParColumna(rowKey);
//      return bean.getGestor().getScriptAbrirVentanaVerDetalle(urx, id);
    ...
    protected GestorBusquedaReferencia getGestorBusquedaIdClaseRecursoParValor() {
        if (this.gestorBusquedaIdClaseRecursoParValor == null) {
            this.gestorBusquedaIdClaseRecursoParValor = new GestorBusquedaReferencia(
                    bean.getFiltroClaRecParDataProvider(),
                    bean.getClaseRecursoParReferenceDataProvider(),
                    FiltroClaRecParCachedRowSetDataProvider2.COLUMNA_ID_CLASE_RECURSO_PAR_VALOR,
    ...
    public String getTextoCampoIdClaseRecursoParValor1() {
        ...
        RowKey rowKey = bean.getGestor().getCurrentRowKey();
        if (bean.getGestor().isFilaNoProcesada()) {
            return STP.getString(bean.getFiltroClaRecParDataProvider().getIdClaseRecursoParValor(rowKey));
        }
        this.getGestorBusquedaIdClaseRecursoParValor().setFiltroBusqueda(this.getFiltroBusquedaIdClaseRecursoParValor());
...
//      RowKey rowKey = bean.getGestor().getCurrentRowKey();
//      String urx = URX2.DETALLE_CLASE_RECURSO_PAR;
//      Long id = bean.getFiltroClaRecParDataProvider().getIdClaseRecursoParValor(rowKey);
//      return bean.getGestor().getScriptAbrirVentanaVerDetalle(urx, id);
        return null;
    }
    ...
    public boolean isNullNumeroOperadorCom() {
        ...
        RowKey rowKey = bean.getGestor().getCurrentRowKey();
        Integer value = bean.getFiltroClaRecParDataProvider().getNumeroOperadorCom(rowKey);
        return value == null;
    }
    ...
    public boolean isGridIdFiltroClaRecParRendered() {
        return true;
    }
