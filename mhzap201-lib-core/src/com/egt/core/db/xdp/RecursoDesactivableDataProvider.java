/*
 * Copyright (C) 2008, EGT Consultores, C.A.
 *
 * Este programa es software libre; usted puede redistribuirlo y/o modificarlo bajo los términos
 * de la licencia "GNU General Public License" publicada por la Fundación "Free Software Foundation".
 *
 * Este programa se distribuye con la esperanza de que pueda ser útil, pero SIN NINGUNA GARANTIA;
 * vea la licencia "GNU General Public License" para obtener mas información.
 *
 */
package com.egt.core.db.xdp;

import com.sun.data.provider.RowKey;

public interface RecursoDesactivableDataProvider {

    /**
     * Getter para propiedad esRecursoInactivo.
     * @return Valor de la propiedad esRecursoInactivo.
     */
    public Integer getEsRecursoInactivo();

    /**
     * Getter para propiedad esRecursoInactivo.
     * @return Valor de la propiedad esRecursoInactivo.
     */
    public Integer getEsRecursoInactivo(RowKey rowKey);

    /**
     * Setter para propiedad esRecursoInactivo.
     * @param esRecursoInactivo Nuevo valor de la propiedad esRecursoInactivo.
     */
    public void setEsRecursoInactivo(Integer esRecursoInactivo);

    /**
     * Setter para propiedad esRecursoInactivo.
     * @param esRecursoInactivo Nuevo valor de la propiedad esRecursoInactivo.
     */
    public void setEsRecursoInactivo(RowKey rowKey, Integer esRecursoInactivo);

    /**
     * Getter para propiedad columnaEsRecursoInactivo.
     * @return Valor de la propiedad columnaEsRecursoInactivo.
     */
    public String getColumnaEsRecursoInactivo();

    /**
     * Getter para propiedad etiquetaEsRecursoInactivo.
     * @return Valor de la propiedad etiquetaEsRecursoInactivo.
     */
    public String getEtiquetaEsRecursoInactivo();
}
