 ...
 */
package com.egt.web.recurso.filtro;
 ...
 */
public class Recurso1 extends AbstractPageBean
...
//      if (this.getRecursoDataProvider().isRecursoDesactivable()) {
//          String clave = this.getRecursoDataProvider().getRecursoDesactivableDataProvider().getColumnaEsRecursoInactivo();
//          Integer valor = IntUtils.valueOf(false);
...
//      }
    ...
    // </editor-fold>
/**/
    private void setFuncionSelect() {
        this.getGestor().track("setFuncionSelect");
        Long funcion = this.getContextoPeticion().getIdentificacionFuncionReferencia();
        if (funcion != null) {
            try {
                String criterios = this.getContextoPeticion().getCriteriosBusqueda();
                this.getListaRecursoDataProvider().setSoloReferencia(true);
                this.getListaRecursoDataProvider().setFuncionSelect(funcion, criterios);
            } catch (Exception ex) {
                this.getGestor().handle(ex);
            }
        }
    ...
    private void setPropiedadesTabla() {
        this.getTable1().setTitle(this.getListaRecursoDataProvider().
                getNombreDominioRecurso());
        this.getTableColumn3().setHeaderText(StringUtils.trimToEmpty(this.getListaRecursoDataProvider().
                getEtiquetaIdentificacionRecurso()).toLowerCase());
        this.getTableColumn4().setHeaderText(StringUtils.trimToEmpty(this.getListaRecursoDataProvider().
                getEtiquetaCodigoRecurso()).toLowerCase());
        this.getTableColumn5().setHeaderText(StringUtils.trimToEmpty(this.getListaRecursoDataProvider().
                getEtiquetaNombreRecurso()).toLowerCase());
        this.getTableColumn6().setHeaderText(StringUtils.trimToEmpty(this.getListaRecursoDataProvider().
                getEtiquetaEsRecursoInactivo()).toLowerCase());
    }

    public Long getIdentificacionRecurso() {
        return this.getListaRecursoDataProvider().getIdentificacionRecurso(this.getGestor().getCurrentRowKey());
    }

    public String getCodigoRecurso() {
        return this.getListaRecursoDataProvider().getCodigoRecurso(this.getGestor().getCurrentRowKey());
    }

    public String getNombreRecurso() {
        return this.getListaRecursoDataProvider().getNombreRecurso(this.getGestor().getCurrentRowKey());
    }

    public String getEsRecursoInactivo() {
        if (this.getListaRecursoDataProvider().isRecursoDesactivable()) {
            boolean b = BitUtils.valueOf(this.getListaRecursoDataProvider().getEsRecursoInactivo(this.getGestor().getCurrentRowKey()));
            return BitUtils.getLabel(b);
        }
        return null;
    }
}
