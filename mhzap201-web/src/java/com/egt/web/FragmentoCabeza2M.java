 ...
 */
public class FragmentoCabeza2M extends AbstractFragmentBean {
...
/**/
    ...
    public String getScriptRetroceder() {
        String script = "";
//      script += "if(history.length>1){history.back()}else{history.go()}";
        script += "if(history.length>1){history.back()}else if(window.opener){window.close()}else{history.go()}";
        script += "return false";
        return script;
    }
