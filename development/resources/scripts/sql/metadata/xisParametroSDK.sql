...
SET ANSI_NULLS ON 
GO
DROP PROCEDURE xisParametroSDK
GO
CREATE PROCEDURE xisParametroSDK AS
BEGIN
DECLARE @rehearsal integer
SELECT 	@rehearsal = id_dual FROM dual 
DECLARE	cursor1 CURSOR FOR
SELECT	DISTINCT RTRIM(colname), RTRIM(colname), dbo.xfnTipoDatoParametro(coltype)
FROM	xdbScriptTables
WHERE	(
		tabname = 'dual' OR
		tabname IN (select codigo_clase_recurso FROM clase_recurso where id_clase_recurso<20000)
		)
