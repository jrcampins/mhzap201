create or replace trigger potencial_ben_before_update
for update on potencial_ben
compound trigger
...
begin
    ...
    --    msg_string := 'No se puede cambiar el tipo de Registro de un Potencial Beneficiario';
    --    raise_application_error(err_number, msg_string, true);
    --end if;
    if(:new.es_potencial_ben_inactivo=1 and :old.es_potencial_ben_inactivo=1) then
        msg_string := 'El Potencial Beneficiario está inactivo. No se puede modificar';
        raise_application_error(err_number, msg_string, true);
    end if;
    /**/
--  :new := sp$potencial_ben.copia_datos_persona(:new);
    if (:new.id_persona is not null) then
        ...
        xnew.codigo_potencial_ben := row_persona.codigo_persona;                   
        xnew.nombre_potencial_ben := row_persona.nombre_persona;                    
        xnew.numero_tipo_reg_pot_ben := :new.numero_tipo_reg_pot_ben;              
        xnew.id_persona := row_persona.id_persona;                                        
        xnew.numero_cedula := row_persona.numero_cedula;                                  
        xnew.letra_cedula := row_persona.letra_cedula;                                    
        xnew.fecha_expedicion_cedula := :new.fecha_expedicion_cedula;              
        xnew.fecha_vencimiento_cedula := :new.fecha_vencimiento_cedula;            
        xnew.primer_nombre := row_persona.primer_nombre;
        xnew.segundo_nombre := row_persona.segundo_nombre;
        xnew.primer_apellido := row_persona.primer_apellido;
        xnew.segundo_apellido := row_persona.segundo_apellido;
        xnew.apellido_casada := row_persona.apellido_casada;
        xnew.fecha_nacimiento := row_persona.fecha_nacimiento;
        xnew.edad := utils.years_since(:new.fecha_nacimiento);
        xnew.es_paraguayo_natural := row_persona.es_paraguayo_natural;
        xnew.numero_cedula := row_persona.numero_cedula;
        xnew.letra_cedula := row_persona.letra_cedula;
        xnew.es_persona_con_empleo := row_persona.es_persona_con_empleo;
        xnew.es_persona_con_deuda := row_persona.es_persona_con_deuda;
        xnew.es_persona_con_pena_judicial := row_persona.es_persona_con_pena_judicial;
        xnew.es_persona_con_cer_vida := row_persona.es_persona_con_cer_vida;
        xnew.es_persona_con_carta_renuncia := row_persona.es_persona_con_carta_renuncia;
        xnew.apodo := row_persona.apodo;
        xnew.fecha_expedicion_cedula := row_persona.fecha_expedicion_cedula;
        xnew.fecha_vencimiento_cedula := row_persona.fecha_vencimiento_cedula;
        xnew.es_indigena := row_persona.es_indigena;
        xnew.id_etnia_indigena := row_persona.id_etnia_indigena;
        xnew.nombre_comunidad_indigena := row_persona.nombre_comunidad_indigena;                                    
...
end potencial_ben_before_update;
/
show errors
