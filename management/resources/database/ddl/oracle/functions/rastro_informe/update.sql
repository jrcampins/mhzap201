create or replace function rastro_informe_update(id number, condicion number, archivo varchar2, mensaje varchar2) return number is
    ts          timestamp := null;
...
begin
    ...
    where   id_rastro_informe = id
    and     numero_condicion_eje_fun < condicion;
    /**/
    if not sql%found then
        msg_string := 'informe ' || id || ' no existe o no se puede colocar en condicion ' || condicion;
