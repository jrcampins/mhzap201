...
--
create or replace function proceso_exp_pen_apr(rastro number,codigo_sime varchar2) return number is
    mensaje varchar2(2000);
    nombre_archivo varchar2(2000);
    retcode number;
begin
    nombre_archivo:='archivo_'||rastro||'.csv';
    mensaje:=sp$proceso.exportar_pen_apr(nombre_archivo, codigo_sime);
    if mensaje like '% Personas con pensión aprobada exportadas' then
