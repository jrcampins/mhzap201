...
AS
SELECT pb.id_potencial_ben,
       pb.nombre_potencial_ben,
       pb.codigo_potencial_ben,
       pb.fecha_nacimiento,
       cast(utils.years_since(pb.fecha_nacimiento) as integer) AS edad,
       pb.letra_cedula,
       pb.numero_cedula,pb.numero_cedula||utils.concat_or_empty('-',pb.letra_cedula) as cedula,
       pb.fecha_expedicion_cedula, 
       pb.fecha_vencimiento_cedula,
       ...
       pb.manzana as manzana, 
       ta.numero_tipo_area,
       ta.codigo_tipo_area as area,
       pb.direccion, 
       case when pb.es_persona_con_jubilacion=1 then 'SI' else 'NO' end as tiene_jubilacion,
