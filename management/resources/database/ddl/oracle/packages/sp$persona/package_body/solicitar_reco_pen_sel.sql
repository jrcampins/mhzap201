--
--Descripción: Este procedimiento solicita reconsideración a una selección
--@param usuario_proceso: id del usuario que ejecuta el proceso
--@param observaciones: código del sime
...
begin
    ...
    if not sql%found then
        ...
        loop
            ...
            exception
                when others then
                    msg_string :='No se pudo solicitar reconsideración a persona '||row_persona_anotada.codigo_persona||'-'||row_persona_anotada.nombre_persona||': '||SQLERRM;
                    raise_application_error(err_number, msg_string, true);
            end;
        end loop;
        delete from persona_anotada where id_usuario=usuario_proceso;
        mensaje:='Se solicitó reconsideración  a '||conta_persona||' Personas. Se limpió la selección del usuario';
        return mensaje;
    end if;
end;