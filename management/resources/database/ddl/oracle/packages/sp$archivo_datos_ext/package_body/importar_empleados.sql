-- 
-- Este procedimiento importa un archivo de datos externos, particularmente un listado de empleados
...
begin
    ...
    if codigo_archivo is null then
        ...
        if sql%found then
            if tipo_arch!='201' then
                msg_string := 'Archivo ' || codigo_archivo || ' NO es de empleados ';
                raise_application_error(err_number, msg_string, true);
            end if;
        end if;
    end if;
    ...
    sp$100.create_csv_log_imp_emp(nombre_archivo, 'WE8ISO8859P1', '1', ';', '"');
    insert into log_imp_emp
           (id_log_imp_emp,
            cedula,
            primer_nombre,
            segundo_nombre,
    ...
    --
    for empleado in (select * from log_imp_emp where es_importado=0 and observacion is null)
    loop
        begin
            id_persona_act:=sp$utils.extract_id_persona(empleado.cedula,empleado.primer_nombre,empleado.segundo_nombre,empleado.primer_apellido,empleado.segundo_apellido,empleado.apellido_casada);
