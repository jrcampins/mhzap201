-- 
-- Este procedimiento importa un archivo de datos externos, particularmente un listado de personas con empleo
...
--
create or replace function importar_empleados(nombre_archivo varchar,codigo_archivo varchar) returns integer as $$
declare
    retorno integer:=0;
    path_domain varchar:='';
...
begin
    ...
    for empleado in select * from log_imp_emp
                    where importado is null loop
        begin
            id_persona_act:=extract_id_persona(empleado.cedula,empleado.primer_nombre,empleado.segundo_nombre,empleado.primer_apellido,empleado.segundo_apellido,empleado.apellido_casada);
