-- 
-- Este procedimiento importa un archivo de datos externos, particularmente un listado de jubilados y pensionados
...
--
create or replace function importar_jubilados_pensionados(nombre_archivo varchar,codigo_archivo varchar) returns integer as $$
declare
    retorno integer:=0;
    path_domain varchar:='';
...
begin
    ...
    execute' copy log_imp_jub
           (cedula,primer_nombre,segundo_nombre,primer_apellido,segundo_apellido,apellido_casada,tipo_registro) from E'''
    ...
    for jubilado in select * from log_imp_jub
                    where importado is null loop
        begin
            id_persona_act:=extract_id_persona(jubilado.cedula,jubilado.primer_nombre,jubilado.segundo_nombre,jubilado.primer_apellido,jubilado.segundo_apellido,jubilado.apellido_casada);
            ...
            --Se registra la inserción en la tabla intermedia
                ...
                if jubilado.tipo_registro ilike '%PENSIONADO%' then
