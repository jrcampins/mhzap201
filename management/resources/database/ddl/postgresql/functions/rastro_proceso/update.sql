create or replace function rastro_proceso_update(rastro bigint, condicion integer, archivo varchar, mensaje varchar) returns integer as $$
declare
	ts timestamp := NULL;
begin
	...
	/**/
	if not found then
		raise exception 'proceso no existe (id=%) o no se puede colocar en condicion %', rastro, condicion;
	end if;
	/**/
	return	0;
end;
