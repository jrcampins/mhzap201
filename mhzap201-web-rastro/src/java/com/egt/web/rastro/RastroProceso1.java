...
import com.egt.data.general.xdp2.RelacionRastroProcesoCachedRowSetDataProvider2;
 ...
 */
public class RastroProceso1 extends AbstractPageBean
    ...
    private void _init() throws Exception {
        ...
        rastroProcesoDataProvider.setCachedRowSet((javax.sql.rowset.CachedRowSet) getValue("#{SessionBean1.rastroProcesoRowSet}"));
    }
    ...
    private RelacionRastroProcesoCachedRowSetDataProvider2 rastroProcesoDataProvider = new RelacionRastroProcesoCachedRowSetDataProvider2();

    public RelacionRastroProcesoCachedRowSetDataProvider2 getRastroProcesoDataProvider() {
        return rastroProcesoDataProvider;
    }

    public void setRastroProcesoDataProvider(RelacionRastroProcesoCachedRowSetDataProvider2 crsdp) {
        this.rastroProcesoDataProvider = crsdp;
    }
    ...
    private final static String ETIQUETA_MIGAS_PAN = Bundle.getString("RastroProceso1_ETIQUETA_MIGAS_PAN");

    private final static String ETIQUETA_CLASE_DETALLE = Bundle.getString("RastroProceso1_ETIQUETA_CLASE_DETALLE");

    private final static String ETIQUETA_CLASE_MAESTRO = Bundle.getString("RastroProceso1_ETIQUETA_CLASE_MAESTRO");

    @Override
    public RecursoCachedRowSetDataProvider getRecursoDataProvider() {
        return this.getRastroProcesoDataProvider();
    }

    @Override
    public long getFuncionConsultarRecurso() {
        return RelacionRastroProcesoCachedRowSetDataProvider2.FUNCION_CONSULTAR_RELACION_RASTRO_PROCESO;
    }

    @Override
    public long getFuncionCrearRecurso() {
        return -1; // return RelacionRastroProcesoCachedRowSetDataProvider2.FUNCION_CREAR_RASTRO_PROCESO;
    }

    @Override
    public long getFuncionModificarRecurso() {
        return -1; // return RelacionRastroProcesoCachedRowSetDataProvider2.FUNCION_MODIFICAR_RASTRO_PROCESO;
    }

    @Override
    public long getFuncionEliminarRecurso() {
        return -1; // return RelacionRastroProcesoCachedRowSetDataProvider2.FUNCION_ELIMINAR_RASTRO_PROCESO;
    }
    ...
    public String getColumnaIdentificacionRecursoMaestro() {
        return null; // return RelacionRastroProcesoCachedRowSetDataProvider2.COLUMNA_ID_RECURSO_MAESTRO_EQUIS;
    }
    ...
    public void consultar(Long filtro) {
        try {
            this.getRastroProcesoDataProvider().consultarRelacionRastroProceso(filtro);
        } catch (Exception ex) {
    ...
    public String getExtensionNombreArchivo() {
        RowKey rowKey = this.getGestor().getCurrentRowKey();
        String str = StringUtils.trimToEmpty(this.getRastroProcesoDataProvider().getNombreArchivo(rowKey));
        int i = str.lastIndexOf('.');
        return i > 0 && i < str.length() ? str.substring(i + 1).toUpperCase() : str;
    }

    public String getUrlNombreArchivo() {
        RowKey rowKey = this.getGestor().getCurrentRowKey();
        String target = this.getRastroProcesoDataProvider().getNombreArchivo(rowKey);
        return JSF.getFileURL(target);
    }

    public String getScriptAbrirVentanaArchivo() {
        if (this.getGestor().vincular()) {
            RowKey rowKey = this.getGestor().getCurrentRowKey();
            String target = this.getRastroProcesoDataProvider().getNombreArchivo(rowKey);
            return JSF.getOpenFileWindowJavaScript(target);
        }
        return null;
    }

    public String getScriptAbrirVentanaDetalleGrupoProceso() {
        if (this.getGestor().vincular()) {
            RowKey rowKey = this.getGestor().getCurrentRowKey();
            String urx = URX2.DETALLE_GRUPO_PROCESO;
            Long id = this.getRastroProcesoDataProvider().getIdGrupoProceso(rowKey);
